...
<div class="col content-body">
    <div class="row justify-content-between m-2">
        <div class="col-3 size-12">Content Management/Content/Chapter/Page</div>
        <div class="col-auto">
            <a href="<?= base_url('content_management/content/chapter/page/add_page/'.$novel['id'].'/'.$novel['chapter_id']) ?>" class="btn btn-primary">Add Content Page</a>
        </div>
    </div>
    ...
    <div class="col mx-2 mb-2 p-2">
        ...
        <div class="row bg-white rounded-2 my-2 p-2">
            ...
            <div class="col">
                ...
                <div class="container-fluid p-0 d-flex align-items-center justify-content-start mt-2">
                    <img width="15px" height="15px" class="mb-1 me-1" src="<?= base_url('image/star.png') ?>" alt="">
                    <?= $novel['rating']== null ? '0.0' : $novel['rating'] ?>
                </div>
                <div class="col mt-2">
                    <p class="badge bg-info">Chapter <?= $novel['chapter_no'].' '.$novel['chapter_name'] ?></p>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Delete Page</h5>
        ...
        <div class="table-responsive-xl">
            <table id="empTable" class="text-center table-borderless">
                ...
                <?php
                $i = 0;
                foreach ($novel_page as $item) { $i++ ?>
                    <tr>
                        <td id="cm"><?= $i ?></td>
                        <td id="cm">Page <?= $item['page_no'] ?></td>
                        <td id="cm"><?= $item['created'] ?></td>
                        <td class="w-25" id="cm">
                            <a href="<?= base_url('content_management/content/chapter/page/edit_page/'.$item['id'].'/'.$novel['id'].'/'.$novel['chapter_id']) ?>" class="btn btn-success size-14">Edit</a>
                            <a href="<?= base_url('content_management/content/chapter/page/view_page/'.$novel['chapter_id'].'/'.$item['id']) ?>" class="btn btn-success size-14">View</a>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#delete').on('click', function(event) {
            ...
            $.ajax({
                url: '<?= base_url('content_management/content/chapter/page/delete_page') ?>',
                method: "POST",
                dataType: "json",
                data: {
                    'id': $('#id').val(),
